...
export function getSubmitHintContent(isEditMode, status) {
  if (!isEditMode) {
    return "เมื่อส่งแล้ว รายการจะเข้าสู่สถานะ รอตรวจสอบ ก่อนเผยแพร่บนเว็บไซต์";
  }

  if (status === "APPROVED") {
    return "เมื่อบันทึกแล้ว ระบบจะใช้ข้อมูลล่าสุดของคุณและเปลี่ยนสถานะรายการกลับเป็น รอตรวจสอบ";
  }

  if (status === "REJECTED") {
    return "เมื่อบันทึกแล้ว รายการนี้จะถูกส่งกลับเข้าสู่สถานะ รอตรวจสอบ ทันที";
  }
